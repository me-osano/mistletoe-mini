...
# mkinitcpio hooks
sudo tee /etc/mkinitcpio.conf.d/mistletoe_hooks.conf >/dev/null <<'EOF'
HOOKS=(base udev keyboard autodetect microcode modconf kms keymap consolefont block encrypt filesystems fsck btrfs)
EOF
