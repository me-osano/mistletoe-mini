#+TITLE: EMO's GNU Emacs Config
#+AUTHOR: Enosh Misonge Osano (EMO)
#+DESCRIPTION: EMO's personal Emacs config.
#+STARTUP: showeverything
#+OPTIONS: toc:2

* IMPORTANT PROGRAMS TO LOAD FIRST

** Beginning the file with comments
...
- Type of git integration for `treemacs-git-mode'.
  ...
  1. `simple', which highlights only files based on their git status & is slightly faster.
  2. `extended', which highlights both files & directories, but requires python.
...
Nix, Go, Lua programming languages
